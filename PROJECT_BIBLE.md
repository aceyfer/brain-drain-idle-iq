...
- **Scene-save embargo OFFICIALLY LIFTED as of `30d83aa` (2026-07-13).** Root cause of instance #4 was CRLF/LF checkout churn (`core.autocrlf=true` vs Unity's LF-only saves), not a genuine Unity resave bug — fixed for real by `.gitattributes` (`eol=lf` on Unity YAML, `.cs`, `.md`, and third-party text types) plus `core.autocrlf false`, landed in `30d83aa`. **What stays permanent regardless of the lift:** the math-verification rule for scene diffs (see the entry above — ordered-subsequence/multiset fileID comparison, never eyeball hunks); one change per commit; Editor-hands scene saves still get diffed and verified before commit, the embargo just no longer requires a *pre-lift full audit gate* on every single one. Note: §18's post-lift commits (`6d7f709` etc.) were `.cs`-only patches, not live Editor scene saves — the actual first post-lift *scene* save is still pending; it's still expected to change at most the same 7 known-benign lines from the recovery-backup audit, but that prediction hasn't been exercised against a real Editor save yet.
