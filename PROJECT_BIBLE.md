...
- **Scene-smuggling instance #4: an Editor save baked unrelated "Win"-window geometry drift and `PointsConversionGroup` fileID churn into a PremiumShop-only deletion.** Attempting to remove `PremiumShopPanel`/`PremiumShopButton`/`PremiumShopManager` via the Editor produced a scene diff where the PremiumShop content was correctly gone, but several unrelated, pre-existing `Win` GameObjects (at least `Building_2`/`Building_8`'s popups) and `PointsConversionGroup` had their fileIDs renumbered and, in at least one case, an actual `m_AnchoredPosition` value changed — none of it attributable to the PremiumShop deletion itself. **Root cause unknown as of 2026-07-10.** Not diagnosed to a specific Unity behavior (unlike the prior 3 instances, each of which had an identified mechanism — coroutine-vs-deactivation timing, stale flag checks, etc.). Worked around by discarding the contaminated save (`git checkout --`) and applying a clean, hand-verified patch instead of the Editor's own output. **Until the root cause is found: no Editor scene save lands in a commit without a full audit first** (see the next entry for the method) — this is now a standing precondition, not a case-by-case judgment call.
- **Audit method upgrade: eyeball diff review is unreliable on `SampleScene.unity`, use math instead.** The scene contains 423 near-identical `Win`-popup blocks (per the instance-#4 investigation); a large deletion elsewhere in the file shifts diff hunk boundaries enough that many of these unrelated, unchanged objects get represented as phantom remove/add pairs — reading the hunks by eye reliably produces false positives (objects that look "changed" but are actually just relocated in the diff) and, worse, can hide a real change inside the noise. Verify scene diffs by **ordered-subsequence + multiset comparison of fileIDs** (extract all `!u!N &ID` headers from both the `-` and `+` sides, take the set difference to find IDs present on only one side — those are the true adds/deletes; anything appearing on both sides is reshuffle noise, not a real change) or by **comparing against an explicitly expected fileID/name set** enumerated before the edit — not by reading hunks top to bottom and judging by eye.
