...
- ~~Remove COGSWorldPortraitUI GameObject...~~ **DONE `2b5919c` (2026-07-16)** — GameObject (scene fileIDs 706123450000000001-005, direct Canvas child) removed from the scene in §19 pass A.
- ~~Delete `_DioramaContainer` + the five `Diorama_*` figure mounts + `DioramaManager.cs` + the Diorama Camera...~~ **DONE `2b5919c` (2026-07-16)** — `_DioramaContainer`, its Transform, the `DioramaManager` component, all 5 mounts (Diorama_0_Outcast through Diorama_4_President), and the Diorama Camera all removed from the scene in §19 pass A.
...
- Delete the root-level `PedestrianContainer` GameObject (scene fileID 1053150925, plain Transform, last SceneRoots entry — NOT the RectTransform one under Canvas/BackgroundRoot wired to `BackgroundPedestrianManager.containerRect`) + its six `Ped1_Prefab`-`Ped6_Prefab` instances + `PedestrianWalkController.cs` + `PedestrianWobble.cs` + .metas. Retired at `06aaa87` (asserted off each boot); physical removal needs Aceyfer's Editor hands + a math-verified scene save. Note the Editor tool `MainUIControllerWireFix.cs` also references both classes and must be updated (or was already §8-batch-guarded) in the same pass. **Scene-object half DONE `2b5919c` (2026-07-16)** — root GameObject + all six `PedN_Prefab` instances removed from the scene in §19 pass A. **Still pending (passes B/C):** `PedestrianWalkController.cs`/`PedestrianWobble.cs` deletion + `.metas`, and the `MainUIControllerWireFix.cs` reference update/removal.
