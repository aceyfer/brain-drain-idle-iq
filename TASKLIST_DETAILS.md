...
- PLACEHOLDER SWEEP — COGS HALF RESOLVED 2026-08-06. `Assets/_Game/Art/COGS/` held six PlaceholderArtGenerator stubs (COGS_0_Neutral through COGS_5_Unhinged, ~1.5KB each) with zero references anywhere in the project. Deleted. Note this folder was entirely separate from the real COGS portraits in `Assets/_Game/Sprites/Characters/COGs/`, which are final and untouched. `Assets/_Game/Scripts/Editor/PlaceholderArtGenerator.cs` is Editor-only and self-referencing and was deliberately KEPT — retiring the tool is a separate decision. Be aware the tool writes to both `Art/COGS` and `Art/PlayerCharacter`, so running it again will recreate the deleted folder.
- PLAYER CHARACTER ART IS OUTSTANDING AND LIVE — NOT CLEANUP, REAL ART DEBT. Discovered 2026-08-06 while verifying the placeholder sweep. `Assets/_Game/Art/PlayerCharacter/` holds five PlaceholderArtGenerator stubs (PlayerCharacter_0_DimOutline, _1_CyanOutline, _2_MagentaOutline, _3_GoldOutline, _4_WhiteHotOutline, ~1.6KB each) and every one of them IS REFERENCED AND RENDERING. Each is wired to a `CharacterAppearanceStage_*` ScriptableObject in `Assets/_Game/Character/`, and `PlayerCharacter_0_DimOutline` is additionally referenced directly by `Assets/Scenes/SampleScene.unity`. This means the player's own avatar — the element on screen for the entire session — is currently drawn with generator stubs across all five appearance stages. DO NOT DELETE THESE. They must be REPLACED, not removed. The good news is that replacement is clean: the ScriptableObjects and all wiring already exist, so this is a sprite-reference swap in five assets with no new systems required. Art direction is an escalating outline ramp (dim to cyan to magenta to gold to white-hot) matching the character's progression. Per the 2026-08-06 Unity AI capability probe, character work belongs to Leonardo rather than Unity AI, because Unity AI self-reports 20-40% visual drift across a six-image series which is disqualifying for a consistent character. Needs a proper Leonardo prompt set before any generation.
- BG1 fixed in §6.
...
## §39 Editor click-to-select broken — INVESTIGATED 2026-08-26, NOT FIXED
...
**Status:** root cause narrowed to Editor-local session state, not yet fixed (needs a live-Editor check that can't be performed from here).
